...
export default async function RunDetailPage({ params }: RunDetailPageProps) {
  ...
  const topDrifted = driftedMetrics.length
    ...
    : run.topFeatures.filter((item) => item.drifted).slice(0, 5).map((item) => ({
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="space-y-6">
        ...
        <div className="rounded-lg border border-[#E5E5E5] bg-[#F4F4F4] p-6">
          <h2 className="mb-3 text-lg font-bold text-nordea-navy">Source Context</h2>
          <div className="flex items-center gap-3">
            <div className={`rounded-full px-3 py-1.5 ${run.sourceMode === "Live" ? "bg-nordea-teal" : "bg-[#9CA3AF]"}`}>
              <span className="text-xs font-medium text-white">{run.sourceMode}</span>
            </div>
            {run.sourceMode === "Mock" ? <span className="text-sm text-[#6B7280]">Reason: mock fallback active</span> : null}
          </div>
          {run.sourceMode === "Mock" ? (
            <div className="mt-3 flex items-start gap-2 rounded-lg border border-[#F59E0B] bg-[#F59E0B]/10 p-3">
              ...
              <p className="text-sm text-nordea-navy">Mock Mode Active - Using simulated data</p>
            </div>
          ) : null}
        </div>

        <div className="rounded-lg border border-[#E5E5E5] bg-white p-6">
          <h2 className="mb-4 text-lg font-bold text-nordea-navy">Drift Summary</h2>
          ...
          <p className="text-sm text-nordea-navy">
            {run.driftStatus === "red"
              ? "High drift detected. Immediate investigation recommended."
              : run.driftStatus === "yellow"
                ? "Moderate drift detected. Monitor closely."
                : "Minimal drift detected. System performing normally."}
          </p>
        ...
        <div className="rounded-lg border border-[#E5E5E5] bg-white p-6">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-bold text-nordea-navy">Feature Metrics</h2>
        ...
        {run.errorText ? (
          <div className="rounded-lg border-2 border-[#EF4444] bg-white p-6">
            ...
            <div className="space-y-2">
              {tickets.map((ticket) => (
                <div key={ticket.id} className="flex items-center justify-between rounded-lg bg-[#F4F4F4] px-3 py-2">
                  <span className="font-mono text-sm text-nordea-teal">{ticket.id}</span>
                  <div className="flex items-center gap-3">
                    <span
        ...
        <div className="rounded-lg border border-[#E5E5E5] bg-white p-6">
          <h2 className="mb-3 text-lg font-bold text-nordea-navy">Compact Report JSON</h2>
